...
public class GraphPanel extends JPanel {
	...
	private int xPositiveBound;
	private int yPositiveBound;
	private int xNegativeBound;
	private int yNegativeBound;
	...
	public GraphPanel(Equation conic) {
		this.setSize(800,800);
		this.setVisible(true);
	}
	
	public GraphPanel(Point[] points) {
		setBounds(points);
		this.setSize(800,800);
		this.setVisible(true);
	}
	
	public void setBounds(Point[] points) {
		xPositiveBound = (int)points[0].getX();
		yPositiveBound = (int)points[0].getY();
		xNegativeBound = (int)points[0].getX();
		yNegativeBound = (int)points[0].getY();
		for(int i=1; i<points.length; i++) {
			if(points[i].getX() > xPositiveBound ) {
				xPositiveBound = (int)points[i].getX();
			}
			if(points[i].getY() > yPositiveBound ) {
				xPositiveBound = (int)points[i].getY();
			}
			if(points[i].getX() < xNegativeBound ) {
				xNegativeBound = (int)points[i].getX();
			}
			if(points[i].getY() < yNegativeBound ) {
				yNegativeBound = (int)points[i].getY();
			}
		}
		xPositiveBound += 5;
		yPositiveBound += 5;
	...
	public void update(Equation[] equations) {
	...
	// x-axis coord constants
		...
		public void paintComponent(Graphics g) {
			...
			for(int i = 1; i < xCoordNumbers; i++) {
				...
				g2.drawString(Integer.toString(i), X_AXIS_FIRST_X_COORD + (i * xLength) - 3, X_AXIS_Y_COORD + AXIS_STRING_DISTANCE);
			}
			...
			for(int i = 1; i < yCoordNumbers; i++) {
				...
				g2.drawString(Integer.toString(i), Y_AXIS_X_COORD - AXIS_STRING_DISTANCE, Y_AXIS_SECOND_Y_COORD - (i * yLength));
			}
		}
}
